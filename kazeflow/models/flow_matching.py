...
class ConditionalFlowMatching(nn.Module):
    ...
    def forward(
        ...
        # Sample timestep with antithetic pairs for variance reduction.
        # Half the batch gets t, the other half gets (1-t), ensuring every
        # minibatch covers both ends of the ODE trajectory.
        B_half = (B + 1) // 2
        ...
        # CFM loss: Pseudo-Huber (smooth L1/L2 hybrid).
        # Behaves as L2 for small errors, L1 for large — stabilises
        # gradients at extreme timesteps where prediction error is large.
        error = (v_t - u_t) * x_mask
        loss = torch.sqrt(error.square() + 1.0) - 1.0  # delta=1
        loss = loss.sum() / (x_mask.sum() * C)
